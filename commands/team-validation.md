...
### Phase 1: 에이전트 선택 (최소 4개, 최대 11개)

**필수 4개** (모든 프로젝트):
- architect, code-quality, bug-hunter, security

**code-quality 분할**: 대상 파일이 30개 이상이면 code-quality를 2개 투입 (디렉토리 기준으로 파일 반씩 분담). 분할 시에도 총 에이전트 수는 최대 11개를 넘지 않는다.
...
- **풀스택/모노레포**: 필수4 + backend, frontend, dedup, db-optimizer, product, testing + python(Python 존재 시) (10~11개)
